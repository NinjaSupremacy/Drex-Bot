...
const ceemde = JSON.parse(fs.readFileSync('./banco de dados/data/totalcmd.json'));
...
fs.writeFileSync('./funcoes/sticker/data.exif', Buffer.concat([_, __, ___, ____]), function (err) {
console.log(err)
if (err) return console.error(err)
return `./funcoes/sticker/data.exif`
})
}
...
const banner2 = cfonts.render((`Canal: https://youtube.com/@NinjaSupremacy\nDesenvolvedor: Ninja\nContato: +55 91 8257-7902`), {
font: 'console',
align: 'center',
gradient: ['red', 'magenta']
});
 
const banner3 = cfonts.render((`DREX|BOT`), {
  font: 'block',
  align: "center",
  gradient: ['red', 'magenta']
});
...
chyt = `559182577902@s.whatsapp.net`

const usedCommandRecently = new Map();
const isFiltered = (from) => {
  const currentTime = Date.now();
  if (usedCommandRecently.has(from)) {
    const lastUsageTime = usedCommandRecently.get(from);
    const elapsedTime = currentTime - lastUsageTime;
    if (elapsedTime < 10000) { // Tempo de espera inicial de 10 segundos
      return true;
    }
  }
  return false;
...
const addFilter = (from) => {
  const currentTime = Date.now();
  usedCommandRecently.set(from, currentTime);
};
